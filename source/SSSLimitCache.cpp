...
SSSLimitCache::SSSLimitCache(const IDType & player, const IDType & enemyScript, const size_t & iter, const size_t & responses, const size_t & timeLimit)
: _player(player)
...
std::vector<Action> SSSLimitCache::search(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
...
UnitScriptData SSSLimitCache::searchForScripts(const IDType& player, const GameState& state, StateEvalScore& bestScore) {
    cacheLTD2 = new CacheSimpleString();
    Timer t;
    t.start();
...
bool SSSLimitCache::doStratifiedSearch(const IDType & player, const GameState & state, UnitScriptData & currentScriptData, Timer & timer, int & numberTypes, double & timePlayouts, StateEvalScore & bestScore) {
    int numberEvals = 0;
...
IDType SSSLimitCache::calculateInitialSeed(const IDType & player, const GameState & state) {
    IDType bestScript;
...
StateEvalScore SSSLimitCache::eval(const IDType & player, const GameState & state, UnitScriptData & playerScriptsChosen) {
...
void SSSLimitCache::setAllScripts(const IDType & player, const GameState & state, UnitScriptData & data, const IDType & script) {
    for (size_t unitIndex(0); unitIndex < state.numUnits(player); ++unitIndex) {
