#include "PortfolioGreedySearchLongCache.h"

using namespace SparCraft;

PortfolioGreedySearchLongCache::PortfolioGreedySearchLongCache(const IDType & player, const IDType & enemyScript, const size_t & iter, const size_t & responses, const size_t & timeLimit)
: _player(player)
...
PortfolioGreedySearchLongCache::~PortfolioGreedySearchLongCache() {
    delete cacheLTD2;
}

UnitScriptData PortfolioGreedySearchLongCache::searchForScripts(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
    Timer t;
    t.start();
...
std::vector<Action> PortfolioGreedySearchLongCache::search(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
    Timer t;
    t.start();
...
void PortfolioGreedySearchLongCache::doPortfolioSearch(const IDType & player, const GameState & state, UnitScriptData & currentScriptData, Timer & t, StateEvalScore & bestScore) {
    //  Timer t;
    //   t.start();
...
IDType PortfolioGreedySearchLongCache::calculateInitialSeed(const IDType & player, const GameState & state) {
    IDType bestScript;
...
StateEvalScore PortfolioGreedySearchLongCache::eval(const IDType & player, const GameState & state, UnitScriptData & playerScriptsChosen) {
    if (_player == player) {
...
void PortfolioGreedySearchLongCache::setAllScripts(const IDType & player, const GameState & state, UnitScriptData & data, const IDType & script) {
    for (size_t unitIndex(0); unitIndex < state.numUnits(player); ++unitIndex) {
