...
StratifiedPolicySearchCache::StratifiedPolicySearchCache(const IDType & player, const IDType & enemyScript, const size_t & iter, const size_t & responses, const size_t & timeLimit, const size_t & hpLevelDiv)
: _player(player)
...
StratifiedPolicySearchCache::~StratifiedPolicySearchCache() {
    delete cacheLTD2;
}


std::vector<Action> StratifiedPolicySearchCache::search(const IDType & player, const GameState & state) {
    Timer t;
    t.start();
...
void StratifiedPolicySearchCache::doStratifiedSearch(const IDType & player, const GameState & state, UnitScriptData & currentScriptData, Timer & timer) {
  // Timer t;
   // t.start();
...
IDType StratifiedPolicySearchCache::calculateInitialSeed(const IDType & player, const GameState & state) {
    IDType bestScript;
...
StateEvalScore StratifiedPolicySearchCache::eval(const IDType & player, const GameState & state, UnitScriptData & playerScriptsChosen) {
    if (_player == player) {
...
void StratifiedPolicySearchCache::setAllScripts(const IDType & player, const GameState & state, UnitScriptData & data, const IDType & script) {
    for (size_t unitIndex(0); unitIndex < state.numUnits(player); ++unitIndex) {
