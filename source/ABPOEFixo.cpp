...
ABPOEFixo::ABPOEFixo(const IDType& playerID) {
    _playerID = playerID;
    iniciarAlphaBeta();
    poe = new PortfolioOnlineEvolution(_playerID, PlayerModels::NOKDPS, 1, 0, 20);
    lastTime = 0;
}



void ABPOEFixo::getMoves(GameState& state, const MoveArray& moves, std::vector<Action>& moveVec) {
    ...
    if (applyClosestInicialization(unidadesAliadas, unidadesInimigas, state)) {
        ...
        std::cout << " Tempo total do POE "<< ms << std::endl;
     ...
     */
    ms = t.getElapsedTimeInMilliSec();
    printf("\nABPOEFixo   Execução completa %lf ms\n", ms);
...
bool ABPOEFixo::unitsInMoves(GameState& state, const MoveArray& moves) {
...
void ABPOEFixo::controlUnitsForAB(GameState & state, const MoveArray & moves) {
    int numUnits = 4;
    //verifico se as unidades não foram mortas
    std::set<Unit, lex_compare_Poe> tempUnitAbsAB;
    for (auto & un : _unitAbsAB) {
...
void ABPOEFixo::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void ABPOEFixo::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool ABPOEFixo::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void ABPOEFixo::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void ABPOEFixo::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit& ABPOEFixo::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void ABPOEFixo::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void ABPOEFixo::addAttack(const Unit& unitEnemy, const Unit& unitAttack) {
...
Unit ABPOEFixo::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void ABPOEFixo::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray& moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void ABPOEFixo::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void ABPOEFixo::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void ABPOEFixo::copiarStateCleanUnit(GameState& origState, GameState& copState) {
    copState = origState;
...
void ABPOEFixo::iniciarAlphaBeta() {
...
const PositionType ABPOEFixo::getDistManhantan(const Position& pInicial, const Position& pFinal) {
...
const PositionType ABPOEFixo::getDistEuclidiana(const Position& pInicial, const Position& pFinal) {
...
const bool ABPOEFixo::unitNeedMoreAttackForKilled(Unit un) {
...
void ABPOEFixo::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
