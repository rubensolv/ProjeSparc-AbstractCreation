...
namespace SparCraft {
    ...
    class SABCache : public Player {
        AlphaBetaSearchAbstractSSS * alphaBeta;
        SSSLimitCache * pgs;
    ...
    public:
        SABCache(const IDType & playerID);
        SABCache(const IDType & playerID, int numUnitsAB, std::string controlAbstraction);
        ~SABCache();
        void getMoves(GameState & state, const MoveArray & moves, std::vector<Action> & moveVec);

        IDType getType() {
            return PlayerModels::SABCache;
        }
