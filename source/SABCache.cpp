...
SABCache::SABCache(const IDType& playerID) {
    _playerID = playerID;
    iniciarAlphaBeta();
    pgs = new SSSLimitCache(_playerID, PlayerModels::NOKDPS, 1, 0, 40);
    lastTime = 0;
    numUnits = 7;
}

SABCache::SABCache(const IDType& playerID, int numUnitsAB, std::string controlAbstraction) {
    _playerID = playerID;
    iniciarAlphaBeta();
    pgs = new SSSLimitCache(_playerID, PlayerModels::NOKDPS, 1, 0, 40);
    lastTime = 0;
...
void SABCache::getMoves(GameState& state, const MoveArray& moves, std::vector<Action>& moveVec) {
    //std::cout << "************* INICIO SABCache  **************" << std::endl;
    Timer t;
    t.start();
    moveVec.clear();
...
std::cout << "##################################################" << std::endl;
     */
    //ms = t.getElapsedTimeInMilliSec();
    //printf("\n SABCache   Execução completa %lf ms\n", ms);

    //std::cout << "************* FIM SABCache  **************" << std::endl;
...
StateEvalScore SABCache::eval(std::vector<Action> moveVec, GameState& state) {
...
bool SABCache::unitsInMoves(GameState& state, const MoveArray& moves) {
...
void SABCache::controlUnitsForAB(GameState & state, const MoveArray & moves) {
...
void SABCache::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void SABCache::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool SABCache::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void SABCache::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void SABCache::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit& SABCache::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void SABCache::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void SABCache::addAttack(const Unit& unitEnemy, const Unit& unitAttack) {
...
Unit SABCache::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void SABCache::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray& moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void SABCache::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void SABCache::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void SABCache::copiarStateCleanUnit(GameState& origState, GameState& copState) {
    copState = origState;
...
void SABCache::iniciarAlphaBeta() {

    // convert them to the proper enum types
    int moveOrderingID = 0;
    int evalMethodID = 2; //testar com 1
    int playoutScriptID1 = 8;
    int playoutScriptID2 = 8;
    int playerToMoveID = 1;
    int opponentModelID = 8; //ultima opção oponente model script
    ...
    alphaBeta = new AlphaBetaSearchAbstractSSS(params, TTPtr((TranspositionTable *) NULL));
...
const PositionType SABCache::getDistManhantan(const Position& pInicial, const Position& pFinal) {
...
const PositionType SABCache::getDistEuclidiana(const Position& pInicial, const Position& pFinal) {
...
const bool SABCache::unitNeedMoreAttackForKilled(Unit un) {
...
void SABCache::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
...
void SABCache::iniciarClasseAbstracao(std::string controlAbstraction) {
