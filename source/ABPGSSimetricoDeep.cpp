...
ABPGSSimetricoDeep::ABPGSSimetricoDeep(const IDType& playerID) {
    _playerID = playerID;
    iniciarAlphaBeta();
    ...
    numUnits = 4;
    controlPartidas = 1;
    std::cout << "Instancia da Classe  ABPGSSimetricoDeep " << std::endl;
}

ABPGSSimetricoDeep::ABPGSSimetricoDeep(const IDType& playerID, int numUnitsAB) {
    _playerID = playerID;
    iniciarAlphaBeta();
    pgs = new PortfolioGreedySearchNoTime(_playerID, PlayerModels::NOKDPS, 1, 0, 40);
    lastTime = 0;
...
void ABPGSSimetricoDeep::getMoves(GameState& state, const MoveArray& moves, std::vector<Action>& moveVec) {
...
bool ABPGSSimetricoDeep::unitsInMoves(GameState& state, const MoveArray& moves) {
...
void ABPGSSimetricoDeep::controlUnitsForAB(GameState & state, const MoveArray & moves) {
    //verifico se as unidades não foram mortas
    std::set<Unit, lex_simDeep> tempUnitAbsAB;
    for (auto & un : _unitAbsAB) {
...
void ABPGSSimetricoDeep::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void ABPGSSimetricoDeep::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool ABPGSSimetricoDeep::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void ABPGSSimetricoDeep::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void ABPGSSimetricoDeep::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit& ABPGSSimetricoDeep::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void ABPGSSimetricoDeep::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void ABPGSSimetricoDeep::addAttack(const Unit& unitEnemy, const Unit& unitAttack) {
...
void ABPGSSimetricoDeep::printMapAttack() {
    std::cout << " ********************************** " << std::endl;
...
Unit ABPGSSimetricoDeep::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void ABPGSSimetricoDeep::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray& moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void ABPGSSimetricoDeep::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void ABPGSSimetricoDeep::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void ABPGSSimetricoDeep::copiarStateCleanUnit(GameState& origState, GameState& copState) {
    copState = origState;
...
void ABPGSSimetricoDeep::iniciarAlphaBeta() {
...
const PositionType ABPGSSimetricoDeep::getDistManhantan(const Position& pInicial, const Position& pFinal) {
...
const PositionType ABPGSSimetricoDeep::getDistEuclidiana(const Position& pInicial, const Position& pFinal) {
...
const bool ABPGSSimetricoDeep::unitNeedMoreAttackForKilled(Unit un) {
...
void ABPGSSimetricoDeep::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
...
void ABPGSSimetricoDeep::calculateMedia(GameState& state, MetricDeep& metric) {
...
std::vector<Unit> ABPGSSimetricoDeep::copiaVector(std::vector<Unit> original) {
...
void ABPGSSimetricoDeep::saveMetrics(MetricDeep& metrica, std::vector<Action>& moveVec) {
    mkdir("resultDeep", 0777);
    std::ofstream arquivo;

    std::string nomeArq = "resultDeep/ABPGSSimetricoDeep_" + std::to_string(_playerID) + "_" + metrica.getTimeToString() + "_" + std::to_string(metrica.GetNumberAbstract());
