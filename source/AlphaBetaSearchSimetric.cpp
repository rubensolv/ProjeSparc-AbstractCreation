...
AlphaBetaSearchSimetric::AlphaBetaSearchSimetric(const AlphaBetaSearchParameters & params, TTPtr TT)
: _params(params)
, _currentRootDepth(0)
...
void AlphaBetaSearchSimetric::doSearch(GameState & initialState) {
    _searchTimer.start();
...
void AlphaBetaSearchSimetric::doSearchWithMoves(GameState& initialState, UnitScriptData & UnitScriptData, std::set<IDType> & unitsAB, IDType _playerID, StateEvalScore & bestScore) {
    _searchTimer.start();
...
AlphaBetaValue AlphaBetaSearchSimetric::IDAlphaBetaWithPGS(GameState & initialState, const size_t & maxDepth, UnitScriptData & UnitScriptData, std::set<IDType> & unitsAB, IDType _playerID) {
    AlphaBetaValue val;
...
AlphaBetaValue AlphaBetaSearchSimetric::IDAlphaBeta(GameState & initialState, const size_t & maxDepth) {
    AlphaBetaValue val;
    _results.nodesExpanded = 0;
    _results.maxDepthReached = 0;
...
void AlphaBetaSearchSimetric::TTsave(GameState & state, const StateEvalScore & value, const StateEvalScore & alpha, const StateEvalScore & beta, const size_t & depth,
...
TTLookupValue AlphaBetaSearchSimetric::TTlookup(const GameState & state, StateEvalScore & alpha, StateEvalScore & beta, const size_t & depth) {
    TTEntry * entry = _TT->lookupScan(state.calculateHash(0), state.calculateHash(1));
...
const bool AlphaBetaSearchSimetric::searchTimeOut() {
    return (_params.timeLimit() && (_results.nodesExpanded % 50 == 0) && (_searchTimer.getElapsedTimeInMilliSec() >= _params.timeLimit()));
}

const bool AlphaBetaSearchSimetric::terminalState(GameState & state, const size_t & depth) const {
    return (depth <= 0 || state.isTerminal());
}

const AlphaBetaMove & AlphaBetaSearchSimetric::getAlphaBetaMove(const TTLookupValue & TTval, const IDType & playerToMove) const {
...
void AlphaBetaSearchSimetric::generateOrderedMoves(GameState & state, MoveArray & moves, const TTLookupValue & TTval, const IDType & playerToMove, const size_t & depth) {
...
bool AlphaBetaSearchSimetric::getNextMoveVec(IDType playerToMove, MoveArray & moves, const size_t & moveNumber, const TTLookupValue & TTval, const size_t & depth, std::vector<Action> & moveVec) const {
...
const IDType AlphaBetaSearchSimetric::getPlayerToMove(GameState & state, const size_t & depth, const IDType & lastPlayerToMove, const bool isFirstSimMove) const {
...
const bool AlphaBetaSearchSimetric::isTranspositionLookupState(GameState & state, const std::vector<Action> * firstSimMove) const {
    return !state.bothCanMove() || (state.bothCanMove() && !firstSimMove);
}

AlphaBetaValue AlphaBetaSearchSimetric::alphaBetaWithPGS(GameState & state, size_t depth, const IDType lastPlayerToMove, std::vector<Action> * prevSimMove, StateEvalScore alpha, StateEvalScore beta, UnitScriptData & unitScriptData, std::set<IDType> & unitsAB, IDType _playerID) {
    // update statistics
    ...
    if (playerToMove == _playerID) {
        UnitScriptData scriptTemp;
        if (depth > 2) {
            const IDType enemyPlayer(state.getEnemy(_playerID));
            for (size_t unitIndex(0); unitIndex < state.numUnits(_playerID); ++unitIndex) {
                scriptTemp.setUnitScript(state.getUnit(_playerID, unitIndex), SparCraft::PlayerModels::NOKDPS);
            }
            //Enemy
            for (size_t unitIndex(0); unitIndex < state.numUnits(enemyPlayer); ++unitIndex) {
                scriptTemp.setUnitScript(state.getUnit(enemyPlayer, unitIndex), SparCraft::PlayerModels::NOKDPS);
            }
        } else {
            scriptTemp = unitScriptData;
        }
        state.generateMoves(movesPgs, playerToMove);
        std::vector<Action> moveVecPgs;
        GameState copy(state);
        scriptTemp.calculateMoves(playerToMove, movesPgs, copy, moveVecPgs);
        ...
        for (size_t unitIndex(0); unitIndex < moves.numUnits(); ++unitIndex) {
            ...
            if ( unitsAB.find(unit.ID()) != unitsAB.end()  ) {
                UnitScriptData tempScript;

                
                //gero para o NOKDPS
                 const IDType enemyPlayer(state.getEnemy(_playerID));
                for (size_t unitIndex(0); unitIndex < state.numUnits(_playerID); ++unitIndex) {
                    tempScript.setUnitScript(state.getUnit(_playerID, unitIndex), SparCraft::PlayerModels::NOKDPS);
                }
                //Enemy
                for (size_t unitIndex(0); unitIndex < state.numUnits(enemyPlayer); ++unitIndex) {
                    tempScript.setUnitScript(state.getUnit(enemyPlayer, unitIndex), SparCraft::PlayerModels::NOKDPS);
                }
                
                std::vector<Action> moveVecKDPS;
                GameState copyKDPS(state);
                tempScript.calculateMoves(playerToMove, movesPgs, copyKDPS, moveVecKDPS);
                moves.replaceMovimentUnit(moveVecKDPS[unitIndex]);
                
                //gero para o Kiter
                 for (size_t unitIndex(0); unitIndex < state.numUnits(_playerID); ++unitIndex) {
                    tempScript.setUnitScript(state.getUnit(_playerID, unitIndex), SparCraft::PlayerModels::KiterDPS);
                }
                //Enemy
                for (size_t unitIndex(0); unitIndex < state.numUnits(enemyPlayer); ++unitIndex) {
                    tempScript.setUnitScript(state.getUnit(enemyPlayer, unitIndex), SparCraft::PlayerModels::KiterDPS);
                }
                
                std::vector<Action> moveVecKiter;
                GameState copyKiter(state);
                tempScript.calculateMoves(playerToMove, movesPgs, copyKiter, moveVecKiter);
                moves.add(moveVecKiter[unitIndex]);
...
AlphaBetaValue AlphaBetaSearchSimetric::alphaBeta(GameState & state, size_t depth, const IDType lastPlayerToMove, std::vector<Action> * prevSimMove, StateEvalScore alpha, StateEvalScore beta) {
    // update statistics
    _results.nodesExpanded++;
...
AlphaBetaSearchResults & AlphaBetaSearchSimetric::getResults() {
    return _results;
}

const IDType AlphaBetaSearchSimetric::getEnemy(const IDType & player) const {
    return (player + 1) % 2;
}

const bool AlphaBetaSearchSimetric::isRoot(const size_t & depth) const {
    return depth == _currentRootDepth;
}

void AlphaBetaSearchSimetric::printTTResults() const {
    printf("\n");
...
void AlphaBetaSearchSimetric::setLimitTime(size_t timeLimit) {
