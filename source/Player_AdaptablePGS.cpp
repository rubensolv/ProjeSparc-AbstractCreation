...
Player_AdaptablePGS::Player_AdaptablePGS(const IDType & playerID) {
    _playerID = playerID;
    _iterations = 1;
    _responses = 0;
...
Player_AdaptablePGS::Player_AdaptablePGS(const IDType & playerID, const IDType & seed, const size_t & iter, const size_t & responses, const size_t & timeLimit) {
    _playerID = playerID;
    _iterations = iter;
...
void Player_AdaptablePGS::getMoves(GameState & state, const MoveArray & moves, std::vector<Action> & moveVec) {
    moveVec.clear();
    ...
    AdaptablePGSPlus pgs(_playerID, _seed, _iterations, _responses, _timeLimit);
...
void Player_AdaptablePGS::updateInfoPGS(AdaptablePGSPlus & pgs, GameState & state) {
