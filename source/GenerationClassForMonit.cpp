...
GenerationClassForMonit::GenerationClassForMonit(const IDType& playerID) {
    _playerID = playerID;
    iniciarAlphaBeta();
...
void GenerationClassForMonit::getMoves(GameState& state, const MoveArray& moves, std::vector<Action>& moveVec) {
    ...
    if (applyClosestInicialization(unidadesAliadas, unidadesInimigas, state)) {
        ...
        if (unitsInMoves(state, moves) and ((40-ms)> 4) ) {
            ...
            if (gABPGS.getState().eval(_playerID, SparCraft::EvaluationMethods::LTD2) >
                ...
                std::cout<<"Escolhemos o AB "<<std::endl;
                std::cout<<"LTD2=  "<< gABPGS.getState().eval(_playerID, SparCraft::EvaluationMethods::LTD2).val()  <<std::endl;
            } else {
                moveVec = moveVecPgs;
                std::cout<<"Escolhemos o PGS "<<std::endl;
                std::cout<<"LTD2=  "<< gPGS.getState().eval(_playerID, SparCraft::EvaluationMethods::LTD2).val()  <<std::endl;
            }

        } else {
            moveVec = moveVecPgs;
            std::cout<<"Escolhemos o PGS "<<std::endl;
            std::cout<<"LTD2=  "<< copy.eval(_playerID, SparCraft::EvaluationMethods::LTD2).val()  <<std::endl;
        }
...
bool GenerationClassForMonit::unitsInMoves(GameState& state, const MoveArray& moves) {
...
void GenerationClassForMonit::controlUnitsForAB(GameState & state, const MoveArray & moves) {
    int numUnits = 4;
    //verifico se as unidades não foram mortas
    std::set<Unit, lex_mon> tempUnitAbsAB;
    for (auto & un : _unitAbsAB) {
...
void GenerationClassForMonit::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void GenerationClassForMonit::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool GenerationClassForMonit::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void GenerationClassForMonit::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void GenerationClassForMonit::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit& GenerationClassForMonit::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void GenerationClassForMonit::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void GenerationClassForMonit::addAttack(const Unit& unitEnemy, const Unit& unitAttack) {
...
void GenerationClassForMonit::printMapAttack() {
    std::cout << " ********************************** " << std::endl;
...
Unit GenerationClassForMonit::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void GenerationClassForMonit::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray& moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void GenerationClassForMonit::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void GenerationClassForMonit::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void GenerationClassForMonit::copiarStateCleanUnit(GameState& origState, GameState& copState) {
    copState = origState;
...
void GenerationClassForMonit::iniciarAlphaBeta() {
...
const PositionType GenerationClassForMonit::getDistManhantan(const Position& pInicial, const Position& pFinal) {
...
const PositionType GenerationClassForMonit::getDistEuclidiana(const Position& pInicial, const Position& pFinal) {
...
const bool GenerationClassForMonit::unitNeedMoreAttackForKilled(Unit un) {
...
void GenerationClassForMonit::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
