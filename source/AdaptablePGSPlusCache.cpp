...
AdaptablePGSPlusCache::AdaptablePGSPlusCache(const IDType & player, const IDType & enemyScript, const size_t & iter, const size_t & responses, const size_t & timeLimit)
: _player(player)
...
AdaptablePGSPlusCache::~AdaptablePGSPlusCache() {
    delete cacheLTD2;
}

UnitScriptData AdaptablePGSPlusCache::searchForScripts(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
    Timer t;
    t.start();
...
std::vector<Action> AdaptablePGSPlusCache::search(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
    Timer t;
    t.start();
...
void AdaptablePGSPlusCache::doPortfolioSearch(const IDType & player, const GameState & state, UnitScriptData & currentScriptData, Timer & t, StateEvalScore & bestScore) {
      //Timer t;
      //t.start();
      double ms; 
...
IDType AdaptablePGSPlusCache::calculateInitialSeed(const IDType & player, const GameState & state) {
    IDType bestScript;
...
StateEvalScore AdaptablePGSPlusCache::eval(const IDType & player, const GameState & state, UnitScriptData & playerScriptsChosen) {
    const IDType enemyPlayer(state.getEnemy(player));
    if (_player == player) {
...
void AdaptablePGSPlusCache::setAllScripts(const IDType & player, const GameState & state, UnitScriptData & data, const IDType & script) {
    for (size_t unitIndex(0); unitIndex < state.numUnits(player); ++unitIndex) {
