...
GenerationClassForDeep::GenerationClassForDeep(const IDType& playerID) {
    _playerID = playerID;
    iniciarAlphaBeta();
    ...
    numUnits = 7;
    controlPartidas = 1;
    std::cout << "Instancia da Classe  GenerationClassForDeep " << std::endl;
}

GenerationClassForDeep::GenerationClassForDeep(const IDType& playerID, int numUnitsAB) {
    _playerID = playerID;
    iniciarAlphaBeta();
    pgs = new PortfolioGreedySearchNoTime(_playerID, PlayerModels::NOKDPS, 1, 0, 40);
    lastTime = 0;
...
bool GenerationClassForDeep::unitsInMoves(GameState& state, const MoveArray & moves) {
...
void GenerationClassForDeep::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void GenerationClassForDeep::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool GenerationClassForDeep::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void GenerationClassForDeep::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void GenerationClassForDeep::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit & GenerationClassForDeep::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void GenerationClassForDeep::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void GenerationClassForDeep::addAttack(const Unit& unitEnemy, const Unit & unitAttack) {
...
void GenerationClassForDeep::printMapAttack() {
    std::cout << " ********************************** " << std::endl;
...
Unit GenerationClassForDeep::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void GenerationClassForDeep::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray & moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void GenerationClassForDeep::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void GenerationClassForDeep::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void GenerationClassForDeep::copiarStateCleanUnit(GameState& origState, GameState & copState) {
    copState = origState;
...
void GenerationClassForDeep::iniciarAlphaBeta() {
...
const PositionType GenerationClassForDeep::getDistManhantan(const Position& pInicial, const Position & pFinal) {
...
const PositionType GenerationClassForDeep::getDistEuclidiana(const Position& pInicial, const Position & pFinal) {
...
const bool GenerationClassForDeep::unitNeedMoreAttackForKilled(Unit un) {
...
void GenerationClassForDeep::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
...
std::vector<Unit> GenerationClassForDeep::copiaVector(std::vector<Unit> original) {
