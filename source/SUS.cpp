...
SUS::SUS(const IDType& playerID, int numUnitsAB, std::string controlAbstraction) {
    _playerID = playerID;
    iniciarAlphaBeta();
    pgs = new AdaptableStratifiedPolicySearchLimit(_playerID, PlayerModels::NOKDPS, 1, 0, 40);
    lastTime = 0;
...
void SUS::getMoves(GameState& state, const MoveArray& moves, std::vector<Action>& moveVec) {
    //std::cout << "************* INICIO SUS  **************" << std::endl;
    Timer t;
    t.start();
    moveVec.clear();
    ...
    if (unitsInMoves(state, moves) and ((40 - ms) > 4) //and (state.numUnits(_playerID) <= numUnits)
        ...
        ABScore = eval(moveVec, copyAB);
        //std::cout << "SUS AB: " << ABScore.val() << " SSS+: " << PGSScore.val() << std::endl;
...
std::cout << "##################################################" << std::endl;
     */
    //ms = t.getElapsedTimeInMilliSec();
    //printf("\n SUS   Execução completa %lf ms\n", ms);

    //std::cout << "************* FIM SUS  **************" << std::endl;
...
StateEvalScore SUS::eval(std::vector<Action> moveVec, GameState& state) {
...
bool SUS::unitsInMoves(GameState& state, const MoveArray& moves) {
...
void SUS::controlUnitsForAB(GameState & state, const MoveArray & moves) {
...
void SUS::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void SUS::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool SUS::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void SUS::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void SUS::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit& SUS::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void SUS::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void SUS::addAttack(const Unit& unitEnemy, const Unit& unitAttack) {
...
Unit SUS::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void SUS::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray& moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void SUS::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void SUS::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void SUS::copiarStateCleanUnit(GameState& origState, GameState& copState) {
    copState = origState;
...
const PositionType SUS::getDistManhantan(const Position& pInicial, const Position& pFinal) {
...
const PositionType SUS::getDistEuclidiana(const Position& pInicial, const Position& pFinal) {
...
const bool SUS::unitNeedMoreAttackForKilled(Unit un) {
...
void SUS::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
...
void SUS::iniciarClasseAbstracao(std::string controlAbstraction) {
