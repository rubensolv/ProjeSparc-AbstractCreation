...
GenerationClassForMetricSample::GenerationClassForMetricSample(const IDType& playerID) {
    _playerID = playerID;
    iniciarAlphaBeta();
    ...
    numUnits = 7;
    _controlNumAbs = 0;
    std::cout << "Instancia da Classe  GenerationClassForMetricSample " << std::endl;
}

GenerationClassForMetricSample::GenerationClassForMetricSample(const IDType& playerID, int numUnitsAB) {
    _playerID = playerID;
    iniciarAlphaBeta();
    pgs = new PortfolioGreedySearchNoTime(_playerID, PlayerModels::NOKDPS, 1, 0, 40);
    lastTime = 0;
...
void GenerationClassForMetricSample::getMoves(GameState& state, const MoveArray& moves, std::vector<Action>& moveVec) {
...
bool GenerationClassForMetricSample::unitsInMoves(GameState& state, const MoveArray & moves) {
...
void GenerationClassForMetricSample::controlUnitsForAB(GameState & state, const MoveArray & moves) {
    //verifico se as unidades não foram mortas
    std::set<Unit, lex_mets> tempUnitAbsAB;
    for (auto & un : _unitAbsAB) {
...
void GenerationClassForMetricSample::analisarAbstractForm(GameState newState, std::vector<Unit> unidadesInimigas) {
 ...
 */
void GenerationClassForMetricSample::addMoreEnemy(GameState& newState, std::vector<Unit>& unInimigas) {
 ...
 */
bool GenerationClassForMetricSample::applyClosestInicialization(std::vector<Unit> & unAliadas, std::vector<Unit> & unInimigas, GameState & state) {
...
void GenerationClassForMetricSample::removeLoseAttacks(GameState& newState, std::vector<Action>& moveVec, GameState & state) {
    _UnReut.clear();
    Unit unAval;
...
void GenerationClassForMetricSample::removeActionInVector(Action& action, std::vector<Action>& moveVec) {
...
Unit & GenerationClassForMetricSample::getCalculateEnemy(GameState& state, std::vector<Unit> unidadesInimigas) {
...
void GenerationClassForMetricSample::doAlphaBeta(GameState & newState, std::vector<Action> & moveVec, GameState & state) {
    //executa a busca
...
void GenerationClassForMetricSample::addAttack(const Unit& unitEnemy, const Unit & unitAttack) {
...
void GenerationClassForMetricSample::printMapAttack() {
    std::cout << " ********************************** " << std::endl;
...
Unit GenerationClassForMetricSample::getEnemyClosestvalid(GameState& state, std::vector<Unit> unidadesInimigas) {
    for (auto & un : unidadesInimigas) {
 ...
 */
void GenerationClassForMetricSample::listaOrdenadaForMoves(const IDType& playerID, const Unit& unidade, GameState& state, std::vector<Unit>& unidades, const MoveArray & moves) {
    unidades.clear();
    //declaração
    Unit t;
...
void GenerationClassForMetricSample::sortUnit(std::vector<Unit>& unidades, const Unit& base, GameState & state) {
 ...
 * passada como referência (inclusive a unidade passada como ponto de referencia)
 */
void GenerationClassForMetricSample::listaOrdenada(const IDType& playerID, const Unit & unidade, GameState& state, std::vector<Unit> & unidades) {
    unidades.clear();
    //declaração
    Unit t;
 ...
 * necessárias.
 */
void GenerationClassForMetricSample::copiarStateCleanUnit(GameState& origState, GameState & copState) {
    copState = origState;
...
void GenerationClassForMetricSample::iniciarAlphaBeta() {
...
const PositionType GenerationClassForMetricSample::getDistManhantan(const Position& pInicial, const Position & pFinal) {
...
const PositionType GenerationClassForMetricSample::getDistEuclidiana(const Position& pInicial, const Position & pFinal) {
...
const bool GenerationClassForMetricSample::unitNeedMoreAttackForKilled(Unit un) {
...
void GenerationClassForMetricSample::removeAttackInUnAttack(Unit enemy, Unit Attacker) {
...
void GenerationClassForMetricSample::split(const std::string& s, std::vector<std::string>& v) {
    v.clear();
    std::string temp ="";
    char delimitador = ';';
...
void GenerationClassForMetricSample::lerAmostragem(std::vector<std::vector<int> > & combinacoes) {
    
    std::string linha;
    std::ifstream arquivo("amostragens/amostra");
    ...
    if(arquivo.is_open()){
        while(! arquivo.eof()){
            ...
            for(auto & num : vTemp){
                combinacao.push_back( atoi(num.c_str()) );
            }
            if(combinacao.size() == 7){
                combinacoes.push_back(combinacao);
            }
        }
    }else{
        std::cout <<"Arquivo de amostra não encontrado!"<<std::endl;
    }
    
}


void GenerationClassForMetricSample::iniciaAbstracao(GameState & state) {
    std::vector<std::vector<int> > combinacoes;

    lerAmostragem(combinacoes);
...
std::vector<Unit> GenerationClassForMetricSample::copiaVector(std::vector<Unit> original) {
...
void GenerationClassForMetricSample::calculateMedia(GameState & state, MetricGAB& metric) {
    PositionType posicaoMedia(0);
    int qtd = 0;
    std::set<Unit, lex_mets> newUnits;
    for (auto & un : _unitAbsAB) {
        if (state.unitExist(_playerID, un.ID())) {
...
void GenerationClassForMetricSample::saveMetrics(MetricGAB & metrica) {
