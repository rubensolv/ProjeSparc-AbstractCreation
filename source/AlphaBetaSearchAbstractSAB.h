...
public:
	...
	AlphaBetaSearchAbstractSAB(const AlphaBetaSearchParameters & params, TTPtr TT = TTPtr((TranspositionTable *)NULL));
	...
	AlphaBetaValue alphaBeta(GameState & state, size_t depth, const IDType lastPlayerToMove, std::vector<Action> * firstSimMove, StateEvalScore alpha, StateEvalScore beta);
        ...
        AlphaBetaValue IDAlphaBetaWithPGS(GameState & initialState, const size_t & maxDepth, UnitScriptData & UnitScriptData, std::set<IDType> & unitsAB, IDType _playerID);
        AlphaBetaValue alphaBetaWithPGS(GameState & state, size_t depth, const IDType lastPlayerToMove, std::vector<Action> * firstSimMove, StateEvalScore alpha, StateEvalScore beta, UnitScriptData & UnitScriptData, std::set<IDType> & unitsAB, IDType _playerID );
	// Transposition Table
