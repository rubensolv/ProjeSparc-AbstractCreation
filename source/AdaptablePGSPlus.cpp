...
AdaptablePGSPlus::AdaptablePGSPlus(const IDType & player, const IDType & enemyScript, const size_t & iter, const size_t & responses, const size_t & timeLimit)
: _player(player)
...
AdaptablePGSPlus::~AdaptablePGSPlus() {
    delete cacheLTD2;
}

UnitScriptData AdaptablePGSPlus::searchForScripts(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
    Timer t;
    t.start();
...
std::vector<Action> AdaptablePGSPlus::search(const IDType & player, const GameState & state, StateEvalScore & bestScore) {
    Timer t;
    t.start();
...
void AdaptablePGSPlus::doPortfolioSearch(const IDType & player, const GameState & state, UnitScriptData & currentScriptData, Timer & t, StateEvalScore & bestScore) {
      //Timer t;
      //t.start();
      double ms; 
...
IDType AdaptablePGSPlus::calculateInitialSeed(const IDType & player, const GameState & state) {
    IDType bestScript;
...
StateEvalScore AdaptablePGSPlus::eval(const IDType & player, const GameState & state, UnitScriptData & playerScriptsChosen) {
/*
    if (_player == player) {
...
void AdaptablePGSPlus::setAllScripts(const IDType & player, const GameState & state, UnitScriptData & data, const IDType & script) {
    for (size_t unitIndex(0); unitIndex < state.numUnits(player); ++unitIndex) {
